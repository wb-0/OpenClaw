...
<body>
    <h2 style="color:#58a6ff;">🦞 DIDI TORONTO AI 指挥部</h2>
    <div id="chat"><div class="msg bot">老板，回车键已加固！请直接输入指令并按回车测试。</div></div>
    ...
    <script>
        async function s(){
            ...
            } catch (err) { c.innerHTML += `<div class="msg bot" style="color:red">信号丢失，请重试。</div>`; }
...
def chat():
    ...
    payload = {"contents": [{"parts": [{"text": "你是小龙虾助手，多伦多Didi Cleaning清洁业务AI指挥。请回复老板：" + user_msg}]}]}
    try:
        res = requests.post(url, json=payload, timeout=20)
        reply = res.json()['candidates'][0]['content']['parts'][0]['text']
    except: reply = "指挥塔响应超时，请再试一次。"
