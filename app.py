...
<head>
    ...
    <style>
        body { background: #0d1117; color: #c9d1d9; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif; margin: 0; padding: 20px; display: flex; flex-direction: column; height: 95vh; }
        ...
        #preview { max-width: 100px; display: none; margin-bottom: 10px; border-radius: 4px; }
    </style>
</head>
<body>
    <h2 style="color:#58a6ff; margin-top:0;">🦞 DIDI TORONTO AI 指挥部</h2>
    <div id="chat">
        <div class="msg bot">老板您好！我是小龙虾。您可以直接跟我聊天，或者上传 North York、Scarborough 等地的现场清洁照片/视频让我分析。</div>
    </div>
    <img id="preview">
    <div class="input-area">
        ...
        <input type="text" id="m" placeholder="输入指令或咨询清洁方案...">
    ...
    <script>
        ...
        async function s(){
            ...
            if(!m.value && !fileBase64) return;
            
            c.innerHTML += `<div class="msg user">${m.value} ${fileBase64 ? '[已上传文件]' : ''}</div>`;
            
            let payload = { p: m.value, file: fileBase64, type: fileType };
            ...
            let d = await r.json();
            c.innerHTML += `<div class="msg bot" style="color:orange">${d.r}</div>`;
...
def chat():
    ...
    # 构建多模态请求内容
    parts = [{"text": f"你是小龙虾，多伦多Didi Cleaning清洁业务的AI指挥。请处理老板指令：{user_msg}"}]
    if file_data:
    ...
    try:
        ...
        return jsonify({"r": f"系统繁忙，请重试。错误: {str(e)}"})
